...
// main RNK Core MapGen button in the left sidebar opens the GM Hub immediately
...
Hooks.on("renderSceneControls", (app, html) => {
  try {
    ...
    const btn = root.querySelector('[data-control="rnk-core-mapgen"]');
    if (!btn || btn.dataset.rnkHubBound === "1") return;
    btn.dataset.rnkHubBound = "1";
    btn.addEventListener("click", () => {
      const hub = window.RnkDungeonGmHub || game.rnkMapper?.RnkDungeonGmHub;
      if (hub) hub.instance.render({ force: true });
      else ui.notifications.error("RNK Core MapGen: GM Hub not initialized.");
    });
  } catch (err) {
    console.error("RNK Core MapGen: Failed to bind scene-control click", err);
  }
});
